...
interface Procedure {
  ...
  description: string
  vigilance:   string[]
  vigilanceQ:  string[]   // questions non résolues pour vigilance
  procedures:  string[]
  normes:      string[]
  documents:   string[]
  checklist:   string[]
...
const PROCEDURES: Procedure[] = [
  ...
  },
  {
    ...
    description:'Importation en exonération totale ou partielle des droits et taxes, sur la base d\'une décision administrative préalable. Chaque code correspond à un régime spécifique d\'exonération. Distinct du Droit Commun (010) qui implique paiement intégral.',
    vigilance:[
  ...
  },
  {
    ...
    description:'Exportation temporaire de marchandises vers l\'étranger pour transformation/perfectionnement, puis réimportation des produits transformés. Droits calculés uniquement sur la valeur ajoutée à l\'étranger (main d\'œuvre + matières étrangères incorporées).',
    vigilance:[
      ...
      'ETPP en suite d\'ATPA (771) : matières ATPA transformées au Maroc puis renvoyées à l\'étranger pour perfectionnement complémentaire',
    ...
    normes:[
      'CDII — Articles ETPP',
      'ALE applicable si perfectionnement dans pays partenaire',
    ],
    documents:[
  ...
  },
  {
    ...
    description:'Export de marchandises marocaines pour perfectionnement à l\'étranger ou en ZAI, puis réimportation ou exportation définitive. Le régime peut se terminer par une vente directe à l\'étranger (codes 680/681) si le produit fini n\'est pas destiné au TA.',
    vigilance:[
  ...
  },
  {
    ...
    description:'Mouvement de marchandises entre deux ZAI distinctes, ou depuis une ZAI vers l\'étranger via transit. La marchandise reste dans la sphère des ZAI sans passer par le TA.',
    vigilance:[
    ...
    conseilIA:'Le transit inter-ZAI est particulièrement utilisé dans les clusters industriels. La coordination logistique avec les bureaux douaniers des deux ZAI est essentielle pour éviter les blocages.',
    questionsOuvertes:[],
  },
  ...
  // ── STOCKAGE ─────────────────────────────────────────────────────────────
  {
    ...
    description:'Régime de stockage sur le TA permettant la suspension des droits et taxes sur des marchandises importées sous AT, dans l\'attente de leur MAC. Géré au niveau des RED (Régies d\'Entrepôts Douaniers). Sert de tampon entre l\'importation et la consommation effective.',
    vigilance:[
      'EIF géré par les RED (Régies d\'Entrepôts Douaniers) — vérifier l\'entrepôt agréé',
    ...
    vigilanceQ:[
      'POINT NON RÉSOLU : Distinction entre code 038 (Entrepôt Industriel Import) et 385 (EIF importation directe) à clarifier avec l\'ADII',
    ],
    procedures:[
    ...
    questionsOuvertes:[
      'POINT NON RÉSOLU : Clarifier codes 038 vs 385 avec l\'ADII',
      'Durée maximale de stockage en EIF',
    ],
  },
  {
    ...
    description:'Entrepôt sous douane créé et géré exclusivement par une société pour ses propres besoins. Même principe que l\'EIF mais sur initiative et gestion privée. Géré au niveau des RED. Usage exclusif du titulaire.',
    vigilance:[
      ...
      'Ces régimes sont sous stockage au niveau des RED',
    ],
  ...
  },
  {
    ...
    normes:['CDII — Articles MEAD','Réglementation ANP pour MEAD portuaires','Conventions de gestion (PortNet, Marsa Maroc…)'],
    ...
    conseilIA:'Le dédouanement par anticipation (avant arrivée du navire) permet d\'éviter les frais de magasinage MEAD et de réduire les délais d\'enlèvement. Activez-le via PortNet 72h avant l\'ETA du navire.',
    questionsOuvertes:[],
  },
  {
    ...
    description:'Zone de stockage logistique sur le territoire assujetti (TA), différente de la ZAI. S\'apparente aux MEAD et ports secs. Pas d\'exonération totale (contrairement à la ZAI) — les droits restent dus à la mise à la consommation. Avantage : infrastructures logistiques optimisées (multimodalité, accès routier/ferroviaire).',
    vigilance:[
      ...
      'Réglementation différente selon l\'opérateur (TMSA, OCP Infra, ONCF Logistique…)',
    ],
    ...
    questionsOuvertes:['Liste des principales zones logistiques TA au Maroc à compléter','Codes DUM spécifiques à ces zones à confirmer'],
  },
  ...
  },
  {
    ...
    description:'Régime spécifique : des produits ou matières issus du TA entrent en ZAI pour un complément d\'ouvraison ou d\'usinage, puis retournent sur le TA. Inverse de l\'ETPP : c\'est le TA qui envoie vers la ZAI pour transformation, puis récupère. Le régime s\'adapte comme un export vers l\'étranger.',
    vigilance:[
    ...
    vigilanceQ:['CODE NON CONFIRMÉ : Valider le code DUM exact pour flux TA→ZAI pour ouvraison→TA avec l\'ADII'],
    procedures:[
    ...
    conseilIA:'L\'ATPP ZAI est un régime hybride peu formalisé. Avant de l\'utiliser, consultez le bureau douanier de la ZAI concernée pour valider la procédure et les codes DUM applicables à votre flux spécifique.',
...
export default function ProceduresPage() {
  ...
  const toggleCheck = (i: number) => {
    setChecks(prev => ({ ...prev, [checkKey(i)]: !prev[checkKey(i)] }))
  }
  ...
  return (
    <Layout variant="inner">
      ...
      <div className="pr-header">
        <div>
          <div className="pr-kicker">MODULE PROCÉDURES</div>
          <div className="pr-title">Procédures Douanières</div>
          <div className="pr-sub">22 procédures · 5 catégories · Codes régimes DUM</div>
        </div>
        <input
          className="pr-search"
          placeholder="Rechercher (titre ou code DUM)…"
          value={search}
      ...
      <div className="pr-cats">
        <button className={`pr-cat${selectedCat === 'ALL' ? ' on' : ''}`}
          onClick={() => setSelectedCat('ALL')} style={selectedCat==='ALL'?{background:'#0A0A0A',color:'#E8C97A',borderColor:'#0A0A0A'}:{}}>
          Tout ({PROCEDURES.length})
        </button>
        {cats.map(c => (
          <button key={c} className={`pr-cat${selectedCat === c ? ' on' : ''}`}
            onClick={() => setSelectedCat(c)}
            style={selectedCat===c?{background:CAT_COLORS[c],color:'#fff',borderColor:CAT_COLORS[c]}:{}}>
      ...
      <div className="pr-layout">
        ...
        <div className="pr-sidebar">
          {filtered.map(p => (
            <button key={p.id} className={`pr-item${p.id === selectedId ? ' on' : ''}`}
              onClick={() => { setSelectedId(p.id); setActiveTab('vigilance') }}
              style={p.id===selectedId?{borderLeftColor:CAT_COLORS[p.categorie]}:{}}>
              <span className="pr-item-cat" style={{background:CAT_BG[p.categorie],color:CAT_COLORS[p.categorie]}}>
                {p.categorie}
              </span>
          ...
          {filtered.length === 0 && (
            <div className="pr-empty">Aucune procédure trouvée</div>
          )}
        </div>
        ...
        <div className="pr-content">
          ...
          <div className="pr-codes">
            {proc.codes.map(c => (
              <span key={c.code} className="pr-code">
                <span className="pr-code-num">{c.code}</span>
                <span className="pr-code-lbl">{c.label}</span>
              </span>
            ))}
          </div>

          {/* Title */}
          <div className="pr-proc-title" style={{color:CAT_COLORS[proc.categorie]}}>
            {proc.titre}
          </div>
          ...
          <div className="pr-tab-row">
            {TABS.map(t => (
              <button key={t.id} className={`pr-tab${activeTab === t.id ? ' on' : ''}`}
            ...
            {tabContent[activeTab].map((item, i) => {
              ...
              return (
                <div key={i} className={`pr-line${isQ ? ' pr-line-q' : ''}`}>
                  <span className="pr-line-bullet" style={{color:isQ?'#BA7517':TABS.find(t=>t.id===activeTab)?.color}}>
                    {isQ ? '❓' : '→'}
                  </span>
          ...
          <div className="pr-check-section">
            <div className="pr-check-header">
              <span className="pr-check-title">Checklist opérationnelle</span>
              <span className="pr-check-count">{checkedCount}/{proc.checklist.length}</span>
            </div>
            <div className="pr-progress-bar">
              <div className="pr-progress-fill" style={{width:`${progress}%`,background:progress===100?'#4CAF7C':CAT_COLORS[proc.categorie]}} />
            </div>
            <div className="pr-checks">
              {proc.checklist.map((item, i) => {
                const checked = !!checks[checkKey(i)]
                return (
                  <div key={i} className={`pr-check-item${checked ? ' checked' : ''}`}
                    onClick={() => toggleCheck(i)}>
                    <div className="pr-checkbox" style={checked?{background:'#4CAF7C',borderColor:'#4CAF7C'}:{}}>
                      {checked && <svg viewBox="0 0 10 10" fill="none" style={{width:10,height:10}}>
          ...
          {/* Conseil IA */}
          <div className="pr-conseil">
            <div className="pr-conseil-label">CONSEIL DOUANE.IA</div>
            <div className="pr-conseil-text">{proc.conseilIA}</div>
          </div>

          {/* Questions ouvertes */}
          {proc.questionsOuvertes.length > 0 && (
            <div className="pr-qo">
              <div className="pr-qo-label">POINTS À COMPLÉTER / QUESTIONS NON RÉSOLUES</div>
              {proc.questionsOuvertes.map((q, i) => (
                <div key={i} className="pr-qo-item">❓ {q}</div>
              ))}
            </div>
          )}
        </div>

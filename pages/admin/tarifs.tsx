...
export default function AdminTarifs() {
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      texte_rag: 'Code SH: ' + form.code_sh + ' | Chapitre: ' + form.code_sh.slice(0,2) + ' | Designation: ' + form.designation_clean + (form.taux_droit != null ? ' | Taux de droit: ' + form.taux_droit + '%' : ''),
    }

    if (modal === 'edit' && selected) {
      const { error } = await supabase.from('tarifs').update(payload).eq('id', selected.id)
      if (error) { showToast('Erreur : ' + error.message) }
      else { showToast('Mis a jour : ' + form.code_sh); setModal(null); search(page) }
    } else {
      const { error } = await supabase.from('tarifs').insert(payload)
      if (error) { showToast('Erreur : ' + error.message) }
      else { showToast('Ajoute : ' + form.code_sh); setModal(null); search(0) }
    }
    setSaving(false)
  }
  ...
  const deleteTarif = async () => {
    ...
    if (error) { showToast('Erreur : ' + error.message) }
    else { showToast('Supprime : ' + selected.code_sh); setModal(null); search(page) }
    setSaving(false)
  }
  ...
  return (
    <AdminLayout>
      ...
      <div style={{ marginBottom: '1rem' }}>
        <input className="search-input" placeholder="Rechercher par code SH ou mot-cle..." value={q} onChange={e => setQ(e.target.value)} />
      </div>
            ...
            <tr key={r.id} style={r.est_hierarchique ? { background: 'var(--bl)' } : {}}>
              ...
              <td>
                <div style={{ display: 'flex', gap: 4 }}>
                  <button className="btn btn-outline btn-sm" onClick={() => openEdit(r)}>Editer</button>
                  <button style={{ padding: '3px 8px', fontSize: 11, background: 'transparent', border: '.5px solid var(--red)', color: 'var(--red)', cursor: 'pointer' }}
                    onClick={() => { setSelected(r); setModal('delete') }}>X</button>
                </div>
              </td>
            </tr>
          ))}
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', gap: '.75rem', marginTop: '1rem', alignItems: 'center' }}>
          <button className="btn btn-outline btn-sm" disabled={page === 0} onClick={() => search(page - 1)}>Precedent</button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div style={{ background: 'var(--white)', width: 540, maxHeight: '90vh', overflow: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.2rem 1.5rem', borderBottom: '.5px solid var(--rule)' }}>
              ...
              <button style={{ background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: 'var(--inkm)' }} onClick={() => setModal(null)}>X</button>
            </div>
            <div style={{ padding: '1.5rem' }}>
              ...
              <div style={{ marginTop: '1rem' }}>
                <label style={{ display: 'block', fontSize: 11, letterSpacing: '.07em', color: 'var(--inks)', marginBottom: 4 }}>DESIGNATION *</label>
                <textarea className="form-textarea" style={{ minHeight: 80 }} {...inp('designation_clean')} placeholder="Description complete de la marchandise" />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '1rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: 11, letterSpacing: '.07em', color: 'var(--inks)', marginBottom: 4 }}>UNITE</label>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--white)', width: 420 }}>
            ...
            <div style={{ padding: '1.5rem', fontSize: 13, color: 'var(--inks)' }}>
              ...
              <span style={{ color: 'var(--red)', fontSize: 12 }}>Cette action est irreversible.</span>
            </div>
            <div style={{ padding: '1rem 1.5rem', borderTop: '.5px solid var(--rule)', display: 'flex', justifyContent: 'flex-end', gap: '.75rem' }}>
              <button className="btn btn-outline" onClick={() => setModal(null)}>Annuler</button>
          ...
          <div style={{ background: 'var(--white)', width: 480 }}>
            ...
            <div style={{ padding: '1.5rem' }}>
              <div style={{ background: 'var(--bl)', padding: '1rem', fontSize: 12, color: 'var(--bm)', marginBottom: '1rem', lineHeight: 1.6 }}>
                ...
                Meme structure que RAG_COMPLET.xlsx. Les codes existants seront mis a jour.
              </div>
              <div style={{ border: '2px dashed var(--rule)', padding: '2rem', textAlign: 'center', cursor: 'pointer', background: importFile ? 'var(--bl)' : 'transparent' }}
                ...
                {importFile
                  ? <div style={{ fontSize: 13, color: 'var(--bd)', fontWeight: 500 }}>OK : {importFile.name}</div>
                  : <div style={{ fontSize: 13, color: 'var(--inkm)' }}>Cliquez pour selectionner un fichier .xlsx ou .csv</div>}
              </div>
              {importProgress && (
            ...
            <div style={{ padding: '1rem 1.5rem', borderTop: '.5px solid var(--rule)', display: 'flex', justifyContent: 'flex-end', gap: '.75rem' }}>
              ...
              <button className="btn btn-primary" disabled={!importFile}
                onClick={() => setImportProgress('Pour importer, utilisez le script import_supabase_v3.py avec votre nouveau fichier. Import via navigateur disponible en Phase 3.')}>
                Lancer l'import
              </button>
            </div>
          </div>
